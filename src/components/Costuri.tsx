...
const Costuri: React.FC<CosturiProps> = ({ userId, userRole }) => {
    ...
    const fetchSpectacoleData = async (startDate?: string, endDate?: string) => {
        ...
        try {
            ...
            fetchColaboratorNames(fetchedData);
        } catch (error) {
    ...
    const spectacoleColumns = [
        { title: 'Spectacol', dataIndex: 'titlu', key: 'titlu', align: 'center' as const, sorter: (a: Spectacol, b: Spectacol) => a.titlu.localeCompare(b.titlu), defaultSortOrder: 'ascend' as const},
            ...
            <Table
                ...
                footer={() => (
                    <div style={{ textAlign: 'right' }}>
                        <Typography.Text strong>Total costuri: {calculateTotalCosturi()} lei</Typography.Text>
                    </div>
                )}
            />
        </div>
